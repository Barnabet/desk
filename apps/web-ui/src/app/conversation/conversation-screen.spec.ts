...
describe('ConversationScreen', () => {
  ...
  it('renders a long conversation from the newest messages at once, and older ones on demand', async () => {
    ...
    await mount(long, { ...initialGlobalState(), connection: { status: 'live' } }, {}, 'desk:events');
  ...
  it('shows thumbnails of the images Desk looked at under its tool calls, and opens one larger', async () => {
    ...
    const bridge = await setup({ 'attachments.get': () => 'data:image/png;base64,ZnVsbA==' }, viewed);
...
describe('messages in the conversation', () => {
  ...
  it("lists a digest pair by pair when expanded, each line opening the pair's sheet", async () => {
    await show([...team(), msg(5, 'a', 'f', 'question', 'Is the login page ready?', minutesAgo(3), { tracked: true }), msg(6, 'f', 'a', 'note', 'Renamed the token field.', minutesAgo(2))]);
    ...
    const sheet = await screen.findByRole('dialog', { name: 'Auth API ⇄ Frontend' });
  ...
  it('labels lanes and plan stops with what threads wait on, "you" only for an attention item, and who is answering', async () => {
    ...
    const plan = {
      project_id: 'p',
      items: [
        { id: '1', title: 'Login API', status: 'in_progress' as const, thread_ids: ['a'], notes: '' },
        { id: '2', title: 'Login page', status: 'in_progress' as const, thread_ids: ['f'], notes: '' },
      ],
      updated_at: 't',
    };
    await show(
  ...
  it("offers Resume on the pause notice while the project's paused item exists", async () => {
    ...
    const bridge = await show([...team(), notice], [paused], { 'attention.dismiss': () => ({ ok: true }) });
    ...
    TestBed.inject(GlobalStore).set((g) => ({ ...g, attention: [] }));
  ...
  it("opens the pair sheet from the counterpart's name on Desk's message rows, and keeps a notice's link", async () => {
    ...
    const sheet = await screen.findByRole('dialog', { name: 'Auth API ⇄ Desk' });
