...
10. **There is no `/v1/shutdown`, and a SIGTERM is undone by `KeepAlive`.** Fixed: `DaemonManager` moves to `@desk/bff/server` with a `web` mode that uses `launchctl` when the plist exists and runs deskd for desk web's data dir (§3).
