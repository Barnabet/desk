...
Global shortcuts: ⌘K (palette: places, projects, threads, skills, turning built-in skills off and on, library titles, and memory search across projects), ⌘P (project switcher), and ⌘1 to ⌘4 (Map, Attention, Skills, System).
...
| File | Covers |
|---|---|
| `smoke.e2e.test.ts` | Onboarding, connecting, the map, a live tray count, switching Appearance between light and dark, and pasting and dropping files into the chat |
| `flows.e2e.test.ts` | Brief → threads fork (counted on the conversation's folded timeline) → question → approval in Attention → revision loop → report → reopening the done thread with a message → the Threads tab unfolding the lanes → tray popover |
